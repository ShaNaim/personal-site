...
import { LayoutGrid, Clock, Menu, X, Wallet, Tags, Target } from "lucide-react";
...
const FINANCE_PAGES = [
  { to: "/finance", icon: Wallet, label: "Dashboard" },
  { to: "/finance/categories", icon: Tags, label: "Categories" },
  { to: "/finance/budget", icon: Target, label: "Budget" },
] as const;
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-1 ml-8">
        {FINANCE_PAGES.map(({ to, icon: Icon, label }) => {
          const isActive = pathname === to;
          return (
            <Link
              key={to}
              to={to}
              className={`flex items-center gap-2 px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest transition-all duration-200 border cursor-none ${
                isActive
                  ? "border-brand text-brand bg-brand-dim"
                  : "border-transparent text-text-muted hover:text-brand hover:border-brand hover:bg-brand-dim"
              }`}
            >
                ...
                {FINANCE_PAGES.map(({ to, icon: Icon, label }) => {
                  ...
                  return (
                    <Link
                      ...
                      to={to}
                      onClick={() => setMenuOpen(false)}
                      className={`flex items-center gap-3 px-4 py-3 text-sm font-mono uppercase tracking-widest border-b border-stroke/50 group cursor-none transition-all ${
                        isActive
                          ? "text-brand bg-brand-dim"
                          : "text-text hover:text-brand hover:bg-brand-dim"
                      }`}
                    >
                      <Icon className="w-4 h-4 shrink-0" />
                      <span>{label}</span>
                      <div className={`ml-auto w-1 h-1 bg-brand ${isActive ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity`} />
                    </Link>
                  );
                })}
              </div>
